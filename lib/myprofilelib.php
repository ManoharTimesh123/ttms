...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    //Custom changes by Intellegencia added by shiva
    $dbman = $DB->get_manager();
    $user_details = new stdClass();
    $show_userorgdata = false;
    if ($dbman->table_exists('local_user_details')) {
        $customorgcategory = new core_user\output\myprofile\category('customorg', 'Organisation details', 'administration');
        
        $tree->add_category($customorgcategory);
        $show_userorgdata = true;
        $user_details = $DB->get_record('local_user_details', array('userid' => $user->id));
    }
    ...
    //Custom changes by Intellegencia added by shiva
    //write logic for org nodes by shiva
    if($show_userorgdata == 'true'){
        if (!empty($user_details->department)) {
            $depts = explode(',', $user_details->department);
            $depname = array();
            foreach($depts as $dept){
                $depname[] = $DB->get_field('local_departments', 'name', array('id' => $dept));
            }
            $data = implode(', ', $depname);
            $node = new core_user\output\myprofile\node('customorg', 'deparments', get_string('departments', 'local_modality'), null, null,
                $data);
            $tree->add_node($node);
        }

        if (!empty($user_details->schoolid)) {
            $school = $DB->get_field('local_schools', 'name', array('id' => $user_details->schoolid));
            $data = $school;
            $node = new core_user\output\myprofile\node('customorg', 'school', get_string('school', 'local_user_management'), null, null,
                $data);
            $tree->add_node($node);
        }

        if (!empty($user_details->position)) {
            $positions = explode(',', $user_details->position);
            $posname = array();
            foreach($positions as $position){
                $posname[] = $DB->get_field('local_school_positions', 'name', array('id' => $position));
            }
            $data = implode(', ', $posname);
            $node = new core_user\output\myprofile\node('customorg', 'positions', get_string('schoolposition', 'local_modality'), null, null,
                $data);
            $tree->add_node($node);
        }

        if (!empty($user_details->subject)) {
            $subs = explode(',', $user_details->subject);
            $subname = array();
            foreach($subs as $sub){
                $subname[] = $DB->get_field('local_subjects', 'name', array('id' => $sub));
            }
            $data = implode(', ', $subname);
            $node = new core_user\output\myprofile\node('customorg', 'subjects', get_string('subjects', 'local_modality'), null, null,
                $data);
            $tree->add_node($node);
        }

        if (!empty($user_details->dob)) {
            $data = date('d/m/Y', $user_details->dob);
            $node = new core_user\output\myprofile\node('customorg', 'dob', get_string('dob', 'local_modality'), null, null,
                $data);
            $tree->add_node($node);
        }
        ...
        if (!empty($user_details->gender)) {
            ...
            $data = $genderarray[$user_details->gender];
            $node = new core_user\output\myprofile\node('customorg', 'gender', get_string('gender', 'local_modality'), null, null,
                $data);
            $tree->add_node($node);
        }
        ...
        if (!empty($user_details->uid)) {
            ...
            $node = new core_user\output\myprofile\node('customorg', 'uid', get_string('uid', 'local_user_management'), null, null,
                $data);
            $tree->add_node($node);
        }
    }
