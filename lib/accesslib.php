...
function has_capability_in_accessdata($capability, context $context, array &$accessdata) {
    ...
    /* INTG Customization Start : Changed logic to only fetch the roles which uses has switched to. */
    if (!empty($USER->access['rsw'])) {
        ...
        $context_array = explode('/', $context->path);

        if(count($context_array) < 4) {
            $roles = [];
...
/* INTG Customization Start : Adding function to strip html tag and limit character. */
function strip_html_tag_and_limit_character($description, $limit = 100) {
...
/* INTG Customization Start : Adding function to change the date format. */
function customdateformat($format, $date) {
    if ($format == 'DATE_WITH_TIME') {
       $dateformat = 'F j, Y, g:i a';
       return date($dateformat, $date);
    } else if ($format == 'DATE_WITHOUT_TIME') {
       $dateformat = 'F j, Y';
       return date($dateformat, $date);
        ...
        $dateformat = 'Y';
        return date($dateformat, $date);
    } else if ($format == 'HOUR_WITH_MINUTES') {
        $dateformat = 'g:i';
        return date($dateformat, $date);
    } else if ($format == 'DAY_NAME') {
        $dateformat = 'l';
        return date($dateformat, $date);
    } else if ($format == 'TIME') {
        $dateformat = 'g:i a';
...
/* INTG Customization Start : Function to Get the current role of the user. */
function custom_get_current_user_role() {
    global $DB, $USER, $COURSE;

    $roleid = $DB->get_field("role", "id", array("shortname" => 'user'));

    // If role is switched get the switched roleid.
    if (isset($USER->access['rsw']) && count($USER->access['rsw']) > 0) {
...
/* INTG Customization Start : Function to switch role to learner. */
function custom_default_roleswitch_to_learner(){

    global $DB, $USER, $COURSE;

    // Role switch to learner on login by default.
    $context = context_course::instance($COURSE->id);
    $switchable_roles = get_user_roles($context, $USER->id);

    if (count($switchable_roles) == 1) {

        if (isloggedin() and !isguestuser()) {
            $userid = $USER->id;
            $role = custom_get_current_user_role();

            $roleid = $DB->get_field('role', 'id', array('shortname' => 'student'));

            $is_roleswitched = 0;
            if(isset($USER->access['rsw'][$context->path])){
                $is_roleswitched = 1;
            }

            $sessionkey = $USER->sesskey;
            $userkey = isset($SESSION->sess_key) ? isset($SESSION->sess_key) : '';
            if(
                array_key_exists($roleid, $switchable_roles) && !($is_roleswitched) &&
                !is_siteadmin() &&
                ($role->name != 'student') &&
                ($userkey != $sessionkey))
            {
