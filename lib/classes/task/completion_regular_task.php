            ...
            while (1) {
                ...
                if (!empty($completions)) {
                    ...
                    if ($overallstatus) {
                        ...
                        /* INTG Customization Start : Course completion mail trigger via local_customnotification */
                        require_once($CFG->dirroot.'/local/customnotifications/lib.php');
                        course_completion_send_message($params->userid, $params->course, $templatename = 'course_completion',
                                                         $emailtobesent = 1, $smstobesent = 1);
                        $log_data = new stdClass();
                        $log_data->template = $templatename;
                        $log_data->touser = $params->userid;
                        $log_data->params = array('course' => $params->course);
                        ...
                        course_feedback_send_message($params->userid, $params->course,'course_feedback',
                                                        $emailtobesent = 1, $smstobesent = 1);

                        $log_data = new stdClass();
                        $log_data->template = 'course_feedback';
                        $log_data->touser = $params->userid;
                        $log_data->params = array('course' => $params->course);
                        $log_data->emailtobesent = 1;
                        $log_data->smstobesent = 1;
