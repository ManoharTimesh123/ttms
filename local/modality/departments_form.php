...
class departments_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'name', get_string('departmentname', 'local_modality'), 'maxlength="120" size="25"');
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', get_string('required'), 'required', null, 'client');

        $mform->addElement('text', 'code', get_string('departmentcode', 'local_modality'), 'maxlength="120" size="25"');
    ...
    public function validation($data, $files) {
        ...
        $department = $DB->get_record('local_departments', array('code' => $data->code));
        if ($data->id == 0) {
            if ($department) {
                $errors['code'] = get_string('alreadyexists', 'local_modality');
            }
        } else {
            if ($olddepartment = $DB->get_record('local_departments', array('id' => $data->id))) {
                if ($department->id != $olddepartment->id ) {
                    $errors['code'] = get_string('alreadyexists', 'local_modality');
                }
            }
        }
