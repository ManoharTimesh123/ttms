...
/* INTG Customization Start : Add function to get current role of logged in user */
function get_loggedin_user_current_role() {
    ...
    if (is_role_switched($COURSE->id)) {
        ...
        // If only one role, then just get the name of that student role.
        if (count($switchable_roles) == 1) {
            // $role = $DB->get_record("role", array("shortname" => 'student'));
        }
    }

    if (!empty($role) && strpos($role->name, 'SCERT') === false) {
        $loggedinrole = 'Loggedin as: '. $role->name;
    }
