...
/* INTG Customization Start : Adding user query form and finding all latest course for programs */
global $PAGE, $CFG;
$PAGE->requires->jquery();
$PAGE->requires->js('/blocks/user_query/js/uquery.js');
$block = block_instance('user_query');
$footer_user_query = $block->external_user_query();
$latestcourseblock = block_instance('latest_courses');
$footer_col_4_body_dynamic_data = $latestcourseblock->external_latest_courses();

/* INTG Customization Start : Add function to get current role of logged in user */
$loggedinusercurrentrole = get_loggedin_user_current_role();
/* INTG Customization End */

/* INTG Customization Start : Show user login link when required condition met */
$showloginurl = false;
if ($PAGE->pagetype == 'local-self_registration-index' ||
    $PAGE->pagetype == 'login-index'
) {
    $showloginurl = true;
}
/* INTG Customization End */

/* INTG Customization Start : Make dynamic footer exlore link */
require_once($CFG->dirroot.'/user/lib.php');
$footermenuehtml = '';
$customitems = user_convert_text_to_menu_items(get_config('theme_edumy', 'footer_col_3_body'), $PAGE);
$footermenuehtml .= '<ul class="nav">';
foreach ($customitems as $item) {
  $getstring = explode(',', $item->titleidentifier);
  if (!empty($getstring[0] && !empty($getstring[1]))) {
    $footermenuehtml .= '<li><a href="' . $item->url . '">' . get_string($getstring[0], $getstring[1]) . '</a></li>';
  }
}
$footermenuehtml .= '</ul>';
/* INTG Customization End */
...
$templatecontext = [
  /* INTG Customization Start : Adding user query form */
    'footer_user_query' => $footer_user_query,
  /* INTG Customization End */
    'lang_menu' =>  $langMenu,
    ...
    /* INTG Customization Start :  Adding all footer explore item html */
    'footer_col_3_body' => format_text($footermenuehtml, FORMAT_HTML, array('filter' => true, 'noclean' => true)),
    /* INTG Customization End */
    'footer_col_4_title' => format_text(get_config('theme_edumy', 'footer_col_4_title'), FORMAT_HTML, array('filter' => true)),
    /* INTG Customization Start :  Adding all latest course in programs */
    //'footer_col_4_body' => format_text(get_config('theme_edumy', 'footer_col_4_body'), FORMAT_HTML, array('filter' => true, 'noclean' => true)),
    'footer_col_4_body' => format_text($footer_col_4_body_dynamic_data),
     /* INTG Customization End */
    ...
    /* INTG Customization Start : Send user role name vairable to mustache for rendering */
    'user_role' => $loggedinusercurrentrole,
    /* INTG Customization End */
    'user_language' => $USER->lang,
    'user_id' => $USER->id,
    'isloggedin' => $isloggedin == 'TRUE',
    'notloggedin' => $isloggedin == 'FALSE',
    /* INTG Customization Start : Send variable to mustache for rendering */
