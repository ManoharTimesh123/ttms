...
/* INTG Customization Start : userrole comming from selector */
$userrole = optional_param('userrole', false, PARAM_INT);
...
/* INTG Customization Start : Finding all participants according to selected userrole selector */
// Get all responses for further use in viewbyresp and deleteall etc.
// All participants.
if (!empty($userrole)) {
    $respsallparticipants = $questionnaire->get_responses(false,0,$userrole);  
} else {
...
switch ($action) {
   ...
   /* INTG Customization Start : Adding user according to role tab*/
   case 'useratingreport':   // View all responses According to user role.
   /* INTG Customization End */
        $PAGE->set_title(get_string('questionnairereport', 'questionnaire'));
        $PAGE->set_heading(format_string($course->fullname));
        ...
        switch ($action) {
            ...
            case 'vallarsort':
                ...
                break;
            /* INTG Customization Start : Adding user rating role tab */
            case 'useratingreport':
                $SESSION->questionnaire->current_tab = 'useratingreport';
                break;
            /* INTG Customization End */
            default:
    ...
    default:
        ...
        if ($byresponse || $rid) {
            ...
            if ($groupmode > 0) {
                switch ($currentgroupid) {
                    case 0: 
                        /* INTG Customization Start : finding all users according to role tab*/
                        // All participants.
                        if (!empty($userrole)) {
                            $resps = $questionnaire->get_responses(false, 0, $userrole);
                        } else {
                            $resps = $respsallparticipants;
                        }
                        /* INTG Customization End */
        ...
        if ($noresponses) {
            ...
            /* INTG Customization Start : Showing selector for user role and showing user list according to selected role*/
            if ($byresponse) {
                $respinfo = '';
                $groupselect = get_user_role_menu($url->out(), $userrole);
                $respinfo .= isset($groupselect) ? ($groupselect . ' ') : '';
            ...
            if ($outputtarget == 'html') {
                if (!empty($userrole)) {
                    $questionnaire->survey_results_navbar_alpha($rid, $currentgroupid, $cm, $byresponse, $userrole);
                } else {
