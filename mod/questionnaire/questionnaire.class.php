...
class questionnaire {
     ...
     * @return array
     */
    /* INTG Customization Start : finding all participants according to selected participant role*/
    public function get_responses($userid=false, $groupid=0, $userrole = null) {
        global $DB;
        $params = [];
        $groupsql = '';
        $groupcnd = '';
        ...
        if ($this->survey_is_public_master()) {
            $sql = 'SELECT r.* ' .
                ...
                if (!empty($userrole)) {
                    $sql .= ' AND r.provided_by_user_roleid = :userrole';
        ...
        if ($resp && !$blankquestionnaire) {
            if ($userid) {
                if ($user = $DB->get_record('user', array('id' => $userid))) {
                    //* INTG Customization Start : Showing user role with user fullname*/
                    $context    = context_course::instance($this->course->id);
                    $userroledetails = get_user_roles($context, $userid, true);
                    $userrolenames = array_column($userroledetails, 'name');

                    $userrolename = $userrolenames[0];
                    $ruser = fullname($user) . ' - ' . $userrolename;
                    /* INTG Customization End */
                }
            }
    ...
    public function response_insert($responsedata, $userid, $resume=false) {
        ...
        if (empty($responsedata->rid)) {
            // Create a uniqe id for this response.
             /* INTG Customization Start : Finding rated_weightage and rated user role */
             $userrateweightagedata = $this->get_user_rate_weightage_infomation($userid);
             $record->provided_by_user_roleid = $userrateweightagedata['provided_by_user_roleid'];
             $record->received_by_user_roleid = $userrateweightagedata['received_by_user_roleid'];
           //  $record->question_rate_weightage_id = $userrateweightagedata->id;
             /* INTG Customization End */
    ...
    /* INTG Customization Start : finding rate weightage according to rated user role */
    public function get_user_rate_weightage_infomation ($userid) {
        global $DB;
        $context    = context_course::instance($this->course->id);
        $userroledetails = get_user_roles($context, $userid, true);
        $userroleids = array_column($userroledetails, 'roleid');
        $userroleid = $userroleids[0];
        $questionnaireid = $this->id;
        $questionnairassigned = $this->assigned_user_role_id;
        $userinfo = array(
            'provided_by_user_roleid' => $userroleid,
            'received_by_user_roleid' => $questionnairassigned
        );
        return $userinfo;
    }
    /* INTG Customization End */
     ...
     */
   /* INTG Customization Start : finding all users according to role*/
    public function survey_results_navbar_alpha($currrid, $currentgroupid, $cm, $byresponse, $userrole = null) {
        global $CFG, $DB;
        ...
        if (!empty($userrole)) {
            if ($isfullname) {
                $responses = $this->get_responses(false, $currentgroupid, $userrole);
            } else {
                $responses = $this->get_responses(false, 0, $userrole);
            }
        } else {
            if ($isfullname) {
