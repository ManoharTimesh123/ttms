...
function xmldb_questionnaire_upgrade($oldversion=0) {
    ...
    /* INTG Customization Start : Creating new table and some fields in existing table */
    if ($oldversion < 2021062303) {

        // Define field assigned_user_role_id to be added to questionnaire.
        $table1 = new xmldb_table('questionnaire');
        $field1 = new xmldb_field('assigned_user_role_id', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'progressbar');

        // Conditionally launch add field assigned_user_role_id.
        if (!$dbman->field_exists($table1, $field1)) {
            $dbman->add_field($table1, $field1);
        }

        $table2 = new xmldb_table('questionnaire_rate_weightage');
        // Adding fields to table questionnaire_rate_weightage.
        $table2->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table2->add_field('questionnaireid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table2->add_field('provided_by_user_roleid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table2->add_field('received_by_user_roleid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table2->add_field('rate_weightage', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        ...
        $table2->add_key('questionnaireid', XMLDB_KEY_FOREIGN, ['questionnaireid'], 'questionnaire', ['id']);
        ...
        // Define field userid to be added to questionnaire_response.
        $table3 = new xmldb_table('questionnaire_response');
        $field2 = new xmldb_field('provided_by_user_roleid', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'userid');
        $field3 = new xmldb_field('received_by_user_roleid', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'provided_by_user_roleid');
        $field4 = new xmldb_field('question_rate_weightage_id', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'received_by_user_roleid');

        // Conditionally launch add field provided_by_user_roleid , received_by_user_roleid , question_rate_weightage_id .
        if (!$dbman->field_exists($table3, $field2)) {
            $dbman->add_field($table3, $field2);
        }
        if (!$dbman->field_exists($table3, $field3)) {
            $dbman->add_field($table3, $field3);
        }
        if (!$dbman->field_exists($table3, $field4)) {
            $dbman->add_field($table3, $field4);
        }


        // Questionnaire savepoint reached.
        upgrade_mod_savepoint(true, 2021062305, 'questionnaire');
    }


    if ($oldversion < 2023050900) {

        // Define field question_rate_weightage_id to be dropped from questionnaire_response.
        $table1 = new xmldb_table('questionnaire_response');
        $field1 = new xmldb_field('question_rate_weightage_id');

        // Conditionally launch drop field question_rate_weightage_id.
        if ($dbman->field_exists($table1, $field1)) {
            $dbman->drop_field($table1, $field1);
        }

         // Define table questionnaire_rate_weightage to be dropped.
         $table2 = new xmldb_table('questionnaire_rate_weightage');

         // Conditionally launch drop table for questionnaire_rate_weightage.
         if ($dbman->table_exists($table2)) {
             $dbman->drop_table($table2);
         }

        // Questionnaire savepoint reached.
        upgrade_mod_savepoint(true, 2023050900, 'questionnaire');
    }
