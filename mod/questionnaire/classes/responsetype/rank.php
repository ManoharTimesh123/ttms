...
class rank extends responsetype {
     ...
     */
      private function mkrescount($rids, $rows, $sort) {
        ...
        $rsql = '';
        /* INTG Customization Start : Adding custom report according to user role*/
        $ruserrolesql = '';
        if (!empty($rids)) {
            list($rsql, $params) = $DB->get_in_or_equal($rids);
             $ruserrolesql = '  qr.id ' . $rsql;
            $rsql = ' AND response_id ' . $rsql;
        }
        if($SESSION->questionnaire->current_tab == 'useratingreport'){
             $userroleinfos = $this->get_user_role_details($params,$ruserrolesql);
        }
        /* INTG Customization End */
        ...
        if($SESSION->questionnaire->current_tab == 'useratingreport'){
            if(!empty($userroleinfos)){
                foreach ($userroleinfos as $rolename => $roleusercount) {
                     $pagetags->totals->headers[] = (object)['align' => 'center', 'text' => $rolename];
                }
            }
        }
        /* INTG Customization End */
            ...
            if (!preg_match("/^[0-9]{1,3}=/", $content)) {
               ...
               /* INTG Customization Start : Adding custom report according to user role*/
                if($SESSION->questionnaire->current_tab == 'useratingreport'){
                    if(!empty($userroleinfos)){
                        foreach ($userroleinfos as $rolename => $roleusercount) {
                             $totalcols[] = (object)['align' => $header->align, 'text' => $roleusercount];;
                        }
                    }
                }
                /* INTG Customization End */
    ...
    /* INTG Customization Start : Adding custom report according to user role*/
   public function get_user_role_details($params,$condition){
        global $DB;
        $roleoptions = get_user_course_role();
        $sql = 'SELECT qr.provided_by_user_roleid,r.name,count(qr.id) as usercount
        FROM {questionnaire_response} qr 
        JOIN {role} r ON r.id = qr.provided_by_user_roleid
        WHERE '. $condition;
        $sql .= ' Group by qr.provided_by_user_roleid';
        $allroledetails = $DB->get_records_sql($sql, $params);
        $roledata = array();
        foreach ($allroledetails as $roleinfo) {
            $roledata[$roleinfo->name] = $roleinfo->usercount;
        }
        $remainingroledata = array();
        foreach ($roleoptions as  $rolename) {
            if (!array_key_exists($rolename, $roledata)) {
                $remainingroledata[$rolename] = 0;
            }
        }
        $allrloedata = array_merge($roledata, $remainingroledata);
        return $allrloedata;
    }
    /* INTG Customization End */
    /**
