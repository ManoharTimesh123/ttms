...
if ($questionnaire->can_view_all_responses_anytime($grouplogic, $resplogic)) {
    ...
    if (in_array($currenttab, array('vall', 'vresp', 'valldefault', 'vallasort', 'vallarsort', 'deleteall', 'downloadcsv',
                                     'vrespsummary', 'individualresp', 'printresp', 'deleteresp', 'useratingreport'))) {
    ...
    if (in_array($currenttab, array('valldefault',  'vallasort', 'vallarsort', 'deleteall', 'downloadcsv', 'useratingreport'))) {
        ...
        if ($currenttab != 'downloadcsv' && $currenttab != 'deleteall') {
            ...
            $argstr2 = $argstr.'&action=useratingreport&group='.$currentgroupid;
            $row3[] = new tabobject('useratingreport', $CFG->wwwroot.htmlspecialchars('/mod/questionnaire/report.php?'.$argstr2),
                                                        get_string('useratingreport', 'questionnaire'));
        }
