...
/* INTG Customization Start : Finding user role details */
...
/*
 * Print user role menu selector.this function is use to create user single selector.
 */
function get_user_role_menu($urlroot, $userrole) {
    global $OUTPUT;   
    $urlroot = new moodle_url($urlroot);
    $userselectlabel = get_string('pleaseselectuserrole', 'questionnaire');
    $selectuserstr = ['0' => get_string('selectuserrole', 'questionnaire')];
    $roleoptions = get_user_course_role();
    $allselectroledata = $selectuserstr + $roleoptions;
    $select = new single_select($urlroot, 'userrole', $allselectroledata, $userrole, null, 'selectuserrole');
    $select->label = $userselectlabel;
    $output = '';
    $output .= $OUTPUT->render($select);
    $output = '<div class="userroleselector">'. $output .'</div>';
    return $output;
}
/*
 * This function is used to find all users role from a course.
 */
function get_user_course_role() {
    $courseroles = get_roles_for_contextlevels(CONTEXT_COURSE);
    $allroles = get_all_roles();
    //In ttms live site added participant role not participants.
    $allowed_roles = ['student', 'facilitator', 'coordinator', 'observer'];
    $userroleoptions = array();
    foreach ($courseroles as $courserole) { 
        $usercourserole = $allroles[$courserole];
        if (in_array($usercourserole->shortname, $allowed_roles)) {
