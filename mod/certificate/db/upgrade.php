...
function xmldb_certificate_upgrade($oldversion=0) {
    ...
    /* INTG Customization Start : Adding fields for managing the certificate approvals in certificate_issues table . */
    if ($oldversion < 2023060601) {
        // Fix previous upgrades.

        $certificateissuestable = new xmldb_table('certificate_issues');
        $statusfield = new xmldb_field('status', XMLDB_TYPE_TEXT, '255', null, false, null, 0);
        $statuscommentfield = new xmldb_field('statuscomment', XMLDB_TYPE_TEXT, '', null, false, null, 0);
        $statusupdatebyfield = new xmldb_field('statusupdateby', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $statusupdatetimefield = new xmldb_field('statusupdatetime', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');

        if (!$dbman->field_exists($certificateissuestable, $statusfield)) {
            $dbman->add_field($certificateissuestable, $statusfield, $continue = true, $feedback = true);
        }

        if (!$dbman->field_exists($certificateissuestable, $statuscommentfield)) {
            $dbman->add_field($certificateissuestable, $statuscommentfield, $continue = true, $feedback = true);
        }

        if (!$dbman->field_exists($certificateissuestable, $statusupdatebyfield)) {
            $dbman->add_field($certificateissuestable, $statusupdatebyfield, $continue = true, $feedback = true);
        }

        if (!$dbman->field_exists($certificateissuestable, $statusupdatetimefield)) {
            $dbman->add_field($certificateissuestable, $statusupdatetimefield, $continue = true, $feedback = true);
        }

        // Certificate savepoint reached.
        upgrade_mod_savepoint(true, 2023060601, 'certificate');
    }
