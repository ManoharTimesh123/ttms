...
if (empty($action)) { // Not displaying PDF
    ...
    /*
        INTG Customization Start : Certificate available only after certificate approval is done.
    */
    ...
    if ($coursedetails && $coursedetails->modality != $moocmodality) {
        $check = 0;
        $certificateissue = $DB->get_record('certificate_issues', array('userid' => $USER->id,
                                                    'certificateid' => $certificate->id));
        if ($certificateissue->status == 'approve') {
            $check = 1;
        } else if ($certificateissue->status == 'reject') {
            $check = 2;
        }

        if ($check == 1 || is_siteadmin()) {
            ...
            $message = 'Your Certificate is Rejected by coordinator';
            echo html_writer::tag('div', $message, array('class' => 'alert alert-danger text-center'));
        } else {
            $message = 'Your Certificate is not approved by coordinator, Once Approved you can view the certificate here.';
            echo html_writer::tag('div', $message, array('class' => 'alert alert-danger text-center'));
        }
    } else {
        // Show as default if mooc courses.
        if ($attempts = certificate_get_attempts($certificate->id)) {
            echo certificate_print_attempts($course, $certificate, $attempts);
        }
        if ($certificate->delivery == 0)    {
