...
/*
    INTG Customization Start : functions for certificate approval.
*/
/**
 * Get courses for certificate approvals.
 */
function certificate_get_courses_for_approval () {
    global $DB, $USER;

    $moocmodality = $DB->get_field('local_modality', 'id', array('shortname' => 'mooc'));
    $coordinatorroleid = $DB->get_field('role', 'id', array('shortname' => 'coordinator'));
    $sql = "SELECT DISTINCT c.id, c.fullname FROM {user_enrolments} AS ue
            JOIN {enrol} AS e ON e.id = ue.enrolid
            JOIN {course} AS c ON c.id = e.courseid
            JOIN {local_course_details} AS lcd ON lcd.course = c.id
            WHERE c.id > 1 AND c.visible = 1 AND lcd.modality <> :moocmodality";

    $params = ['moocmodality' => $moocmodality];

    if (is_siteadmin()) {
        // Show all trainings for admin.
    } else {
        // Show coordinator courses.
        $sql .= " AND ue.userid = :userid";

        $params['coordinatorroleid'] = $coordinatorroleid;
...
/**
 * show Participants list for approval of course.
 */
function show_participants_certificates($courseid = 0, $group = 0) {
    global $DB, $USER;
    $output = '';
    ...
    $sql = "SELECT DISTINCT ue.userid as id
            FROM {groups_members} as gm
            JOIN {groups} as g ON g.id = gm.groupid
            JOIN {user_enrolments} AS ue ON ue.userid = gm.userid
            JOIN {enrol} AS e ON e.id = ue.enrolid
            JOIN {course} AS c ON c.id = g.courseid
            WHERE c.visible = 1 AND g.courseid = :courseid AND e.roleid = :participant AND ue.userid <> :loggedinuser
            AND g.id = :groupid";
    ...
    $tableheader = array(
        ...
        get_string('status'),
        get_string('comments'),
        get_string('statusupdateby', 'mod_certificate'),
        get_string('statusupdatetime', 'mod_certificate'),
        get_string('action')
    );
    ...
    if (!empty($participants)) {
        foreach ($participants as $participant) {
            ...
            $paricipantcertificatesql = "SELECT certi.* FROM {certificate_issues} AS certi
                                        ...
                                        WHERE cert.course = :courseid AND certi.userid = :participantid";
            ...
            $participantrecord = $DB->get_record('user', array('id' => $participant->id));
            $row[] = fullname($participantrecord);

            if (empty($certificaterecord->status)) {
                if ($usercertificateavailable == 1) {
                    $row[] = 'Ready to Approve/Reject';
                } else {
                    $row[] = 'Activities completion pending';
                }
            } else {
                $row[] = $certificaterecord->status;
            }

            if (empty($certificaterecord->statuscomment)) {
                $row[] = '-';
            } else {
                $row[] = $certificaterecord->statuscomment;
            }

            if (empty($certificaterecord->statusupdateby)) {
                $row[] = '-';
            } else {
                $updateuser = $DB->get_record('user', array('id' => $certificaterecord->statusupdateby));
                $row[] = fullname($updateuser);
            }

            if (empty($certificaterecord->statusupdatetime)) {
                $row[] = '-';
            } else {
                $row[] = date('d/m/Y | H:i', $certificaterecord->statusupdatetime);
            }
            ...
            if (!empty($certificaterecord->status)) {
                ...
                $actionbuttons .= html_writer::link(
                                        'javascript:void(0)',
                                        '<i class="fa fa-thumbs-up m-1"></i>',
                                        array('title' => get_string('approve'), 'class' => 'text-success',
                                            'onclick' => 'certificate_status_comment(
                                                            "Approve",
                                                            '.$participant->id.',
                                                            "'.fullname($participantrecord).'",
                                                            '.$courseid.',
                                                            '.$group.');'));

                $actionbuttons .= html_writer::link(
                                        'javascript:void(0)',
                                        '<i class="fa fa-thumbs-down m-1"></i>',
                                        array('title' => get_string('reject'), 'class' => 'text-danger',
                                            'onclick' => 'certificate_status_comment(
                                                            "Reject",
                                                            '.$participant->id.',
                                                            "'.fullname($participantrecord).'",
        ...
        if ($data) {
            $table->size = array('20%', '15%', '25%', '15%', '15%', '10%');
            ...
            $output .= '<div class="modal fade" id="certificateapprovalpopup" tabindex="-1" role="dialog"
                                            aria-labelledby="certificateapprovalpopup" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                            <h5 class="modal-title" id="certificateapprovaltitle">Certificate Approval</h5>
                            ...
                            <div class="modal-body" id="modal-body">
                                <form action="#" >
                                    ...
                                    <textarea id="certificatecomment" name="comments" cols="50" cols="3"> </textarea>
                                </form>
                            </div>
                            <div class="modal-footer">
                            <button type="button" id="popupsubmit" class="btn btn-secondary">Submit</button>
                            </div>
                        </div>
                        </div>
                    </div>';
...
function update_user_certificate_status($user, $courseid, $postdata) {
    global $DB, $USER;

    $sql = "SELECT cm.instance FROM {course_modules} AS cm
            JOIN {modules} AS m ON m.id = cm.module
            WHERE m.name = :certificate AND cm.course = :courseid";
    ...
    $certificate = $DB->get_field_sql($sql, $params);

    if ($user > 0 && $certificate > 0) {
        if (!$DB->record_exists('certificate_issues', array('userid' => $user, 'certificateid' => $certificate))) {
            $certissue = new stdClass();
            $certissue->certificateid = $certificate;
            $certissue->userid = $user;
            $certissue->code = certificate_generate_code();
            $certissue->timecreated = time();
            $newid = $DB->insert_record('certificate_issues', $certissue, true);
        }
        $record = $DB->get_record('certificate_issues', array('userid' => $user, 'certificateid' => $certificate));
        $data = new stdClass();
        $data->id = $record->id;
        $data->userid = $user;
        $data->certificateid = $certificate;
        $data->status = $postdata->status;
        $data->statuscomment = $postdata->statuscomment;
        $data->statusupdateby = $USER->id;
        $data->statusupdatetime = time();
        $out = $DB->update_record('certificate_issues', $data);
        return $out;
    }
    return false;
}
