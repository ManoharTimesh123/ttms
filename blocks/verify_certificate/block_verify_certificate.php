...
class block_verify_certificate extends block_base {
    ...
    public function get_content() {
        ...
        $this->content->text .= '<form class="d-flex" name="cert" method="post" action="'. $url . '">';
        $this->content->text .= '<input class="w-75 rounded-left verify-code" type="text" id="certcode" name="certnumber" size="20" value="" />';
        $this->content->text .= '<input class="text-white border-0 w-25 rounded-right verify-btn" type="submit" value="'.get_string('validate', 'block_verify_certificate').'"/></form>';
        $this->content->footer = '';
