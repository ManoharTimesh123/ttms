...
if (! $certificates) {
    ...
    echo '<div class="col-md-8 mx-auto card shadow text-center p-3" id="block_verify_certificate"><br>';
    echo '<p class="notVerified">' . get_string('certificate', 'block_verify_certificate')
         . ' "' . $id . '" ' . '</p>';
    echo '<div class="wrapper-box">';
    echo '<div>' .get_string('notfound', 'block_verify_certificate').'</div>';
    echo '<div class="mt-2"><img src="pix/certnotverified.png" border="0" align="center"></div>';
    echo '</div></div>';
    echo $OUTPUT->box_end();
} else {
    echo $OUTPUT->box_start('generalbox boxaligncenter shadow certificate-block rounded  w-75 mx-auto text-center');
    echo "<a title=\""; print_string('printerfriendly', 'certificate');
    echo "\" href=\"#\" onclick=\"window.print ()\" class='d-table ml-auto p-2'>";
    echo "<img src=\"pix/printicon.png\" height=\"40\" width=\"40\"></img></a>";

    // Print Section.
    foreach ($certificates as $certdata) {
        echo '<h5 class = "verified mb-4">' . get_string('certificate', 'block_verify_certificate')
        . ' "' . $certdata->code . '" ' . '</font></b></h5>';
        echo '<div>';
        echo '<div class="userprofilebox clearfix"><div class="profilepicture">';
        echo $OUTPUT->user_picture($certdata, array('size' => 100));
        echo '</div>';
        echo '</div><div>';
        echo '<p class="certificate-user-name mt-4"><b class="px-4 py-2 rounded-pill text-white h5 d-table mx-auto">' . get_string('to', 'block_verify_certificate') . ': </b>' . fullname($certdata) . '</p>';

        $course = $DB->get_record('course', array('id' => $certdata->course));
        if ($course) {
            echo '<p class="certificate-user-award "><b>' . get_string('course', 'block_verify_certificate') . ': </b><br>' . $course->fullname . '</p>';
        }
        ...
        echo '</div><div>';
        echo "<img class='certverified-image' src=\"pix/certverified.png\" border=\"0\" align=\"center\"></img>";
        echo '</div><div>';
        echo '<p class="text-center pb-4">' . get_string('check', 'block_verify_certificate') . '</p>';
    }
    echo $OUTPUT->box_end();
}
echo $OUTPUT->footer();
