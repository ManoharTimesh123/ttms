        ...
        initialise: function() {
            ...
            $(document).ready(function() {
                        ...
                        if (parseInt(data.done, 10) > 0) {
                            ...
                            for (var i in data) {
                                if (data.hasOwnProperty(i)) {
                                    ...
                                    /*
                                    INTG Customization Start : Show organisation hierarchy when
                                    global checkox in unchecked
                                    */
                                    if (data['global'] != 1) {
                                        $('.zone-block').css('display', 'none');
                                        $('.diet-block').css('display', 'none');
                                        $('.school-block').css('display', 'none');
                                        $('#organiation-hirarchy').removeAttr('style');
                                        if (data['districtid'] !== null) {
                                            $("#add_notification_districts option:selected").each(function () {
                                                $(this).removeAttr('selected');
                                            });
                                            $('#add_notification_districts').removeAttr('selected');
                                            var selectedids = data['districtid'].split(',');
                                            for (var dis = 0; dis < selectedids.length; dis++) {
                                                $('#add_notification_districts option[value="'+selectedids[dis]+'"]').attr(
                                                    "selected", "selected"
                                                );
                                            }
                                        }
                                        if (data['zoneid'] !== null) {
                                            $("#add_notification_zones option:selected").each(function () {
                                                $(this).removeAttr('selected');
                                            });
                                            $('.zone-block').removeAttr('style');
                                            var zoneids = data['zoneid'].split(',');
                                            for (var zone = 0; zone < zoneids.length; zone++) {
                                                $('#add_notification_zones option[value="'+zoneids[zone]+'"]').attr(
                                                    "selected", "selected"
                                                );
                                            }
                                        }
                                        if (data['dietid'] !== null) {
                                            $("#add_notification_diet option:selected").each(function () {
                                                $(this).removeAttr('selected');
                                            });
                                            $('.diet-block').removeAttr('style');
                                            var dietids = data['dietid'].split(',');
                                            for (var diet = 0; diet < dietids.length; diet++) {
                                                $('#add_notification_diet option[value="'+dietids[diet]+'"]').attr(
                                                    "selected", "selected"
                                                );
                                            }
                                        }
                                        if (data['schoolid'] !== null) {
                                            $("#add_notification_school option:selected").each(function () {
                                                $(this).removeAttr('selected');
                                            });
                                            $('.school-block').removeAttr('style');
                                            var schoolids = data['schoolid'].split(',');
                                            for (var school = 0; school < schoolids.length; school++) {
                                                $('#add_notification_school option[value="'+schoolids[school]+'"]').attr(
                                                    "selected", "selected"
                                                );
                                            }
                                        }
                                    /* INTG Customization End */
                                        ...
                                        $('#organiation-hirarchy').css('display','none');
                                    }
                ...
                var clearForm = function() {
                    ...
                    refreshPreview();
                    /*
                        INTG Customization Start : Hide organisation hierarchy
