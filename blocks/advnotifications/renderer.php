...
class block_advnotifications_renderer extends plugin_renderer_base
{
    ...
    public function add_notification($params) {
        global $CFG, $DB;
        /* INTG Customization Start : Fetching organization records from DB. */
        $dbman = $DB->get_manager();
        $districtoptionshtml = '';
        if ($dbman->table_exists('local_demo_districts')) {
            $districts = $DB->get_records('local_demo_districts', null, 'id');
            foreach ($districts as $district) {
                $districtoptionshtml .= '<option value="'.$district->id.'">'.$district->name.'</option>';
            }
        }

        $zoneoptionshtml = '';
        if ($dbman->table_exists('local_demo_zones')) {
            $zones = $DB->get_records('local_demo_zones', null, 'id');
        ...
        $dietoptionshtml = '';
        if ($dbman->table_exists('local_demo_diet')) {
            $diets = $DB->get_records('local_demo_diet', null, 'id');
        ...
        $schooloptionshtml = '';
        if ($dbman->table_exists('local_demo_schools')) {
            $schools = $DB->get_records('local_demo_schools', null, 'id');
        ...
        $html .= '<div id="add_notification_wrapper_id" class="add_notification_wrapper">
                    ...
                    <div class="add_notification_form_wrapper">
                        <form id="add_notification_form" action="' . $CFG->wwwroot .
                            ...
                            <!-- INTG Customization Start : Added organization dropdown -->
                            <div class="form-check">
                                <input type="checkbox" id="add_notification_global" class="form-check-input" name="global" checked value="1"/>
                            ...
                            <div id="organiation-hirarchy" style="display:none;">
                                <div class="form-group">
                                    <label for="add_notification_districts" class="form-text">
                                    ' . get_string('district', 'block_advnotifications') . '
                                    </label>
                                    <select multiple id="add_notification_districts" class="form-control districts" name="districts[]" >
                                    '.$districtoptionshtml.'
                                    </select>
                                </div>
                                <div class="form-group zone-block" style="display:none;">
                                    <label for="add_notification_zones" class="form-text">
                                    ' . get_string('zone', 'block_advnotifications') . '
                                    </label>
                                    <select multiple id="add_notification_zones" class="form-control zones" name="zones[]" >
                                    '.$zoneoptionshtml.'
                                    </select>
                                </div>
                                <div class="form-group diet-block" style="display:none;">
                                    <label for="add_notification_diet" class="form-text">
                                    ' . get_string('diet', 'block_advnotifications') . '
                                    </label>
                                    <select multiple id="add_notification_diet" class="form-control diets" name="diets[]" >
                                    '.$dietoptionshtml.'
                                    </select>
                                </div>
                                <div class="form-group school-block" style="display:none;">
                                    <label for="add_notification_school" class="form-text">
                                        ' . get_string('school', 'block_advnotifications') . '
                                    </label>
                                    <select multiple id="add_notification_school" class="form-control schools" name="schools[]" >
                                    '.$schooloptionshtml.'
                                    </select>
                                </div>                      
                            </div>
