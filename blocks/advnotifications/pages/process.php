...
if ($purpose == 'update') {
    ...
    $urow->id = $id;
    /*
        INTG Customization Start : Setting up organization field
    ...
    */
    ...
    if (!isset($_POST['global'])) {
        if (isset($_POST['districts']) && !empty($_POST['districts'])) {
            $districtids = implode(",", $_POST['districts']);
        }
        if (isset($_POST['zones']) && !empty($_POST['zones'])) {
            $zoneids = implode(",", $_POST['zones']);
        }
        if (isset($_POST['diets']) && !empty($_POST['diets'])) {
            $dietids = implode(",", $_POST['diets']);
        }
        if (isset($_POST['schools']) && !empty($_POST['schools'])) {
            $schoolids = implode(",", $_POST['schools']);
        }
    }
    $urow->districtid = $districtids;
    $urow->zoneid = $zoneids;
    $urow->dietid = $dietids;
    $urow->schoolid = $schoolids;
    /*
    ...
    /*
        INTG Customization Start : Execute code blog only
        If greater than 0.
    */
    if ($blockinstance > -1) {
        $params = [
...
if ($purpose == "add") {
    ...
    */
    ...
    $row->districtid = $districtids;
    $row->zoneid = $zoneids;
    $row->dietid = $dietids;
    $row->schoolid = $schoolids;
    /*
        INTG Customization End
    */
    $row->title = $title;
