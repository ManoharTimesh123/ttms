...
class block_wall {
    ...
    public static function approvedposts() {
        ...
        $getapprovedpostsql = <<<SQL_QUERY
            select p.*, CONCAT(u.firstname, ' ', u.lastname) as name from {local_wall_posts} p
    ...
    public static function getpostcomment($postid) {
        ...
        $getpostcommentsql = <<<SQL_QUERY
            ...
            INNER JOIN lms_user u ON  pc.createdby = u.id
    ...
    public static function getuploadedfileurl($post, $contextid) {
        global $CFG;
        $postid = $post['id'];
        $fs = get_file_storage();
