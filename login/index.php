...
/* INTG Customization Start : Validate captcha field  */
if (!empty($frm->username) && !empty($frm->password)) {
    ...
    if (isset($formsubmitteddata['g-recaptcha-response']) && !empty($formsubmitteddata['g-recaptcha-response'])) {
        ...
        $errormsg = 'Please check the recaptcha checkbox';
        $errorcode = 3;
    }
}
/* INTG Customization End */
...
if (isloggedin() and !isguestuser()) {
    ...
    /* INTG Customization Start : Setup catptcha variable to be rendered in template  */
    $loginform->humanvarification = recaptcha_get_challenge_html(RECAPTCHA_API_URL, $CFG->recaptchapublickey, null);
    /* INTG Customization End  */
