...
/* INTG Customization Start: Changes for showing only assigned roles. */
global $USER;
...
if ($switchrole > 0 && has_capability('moodle/role:switchroles', $context)) {
    ...
    /* INTG Customization Start: Changes for showing only assigned roles to the loggedin user. */
    $assignedroles = get_user_roles($context, $USER->id);

    if (count($assignedroles) > 1) {
        $newroles = array();
        $currentrole = custom_get_current_user_role();

        if($assignedroles) {
            foreach ($assignedroles as $assignedrole) {
                if ($currentrole->id != $assignedrole->roleid) {
                    $newroles[$assignedrole->roleid] = $assignedrole->name;
                }
            }
        }
        ...
        redirect(new moodle_url('/'), 'Only one role assigned to the user');
        exit;
    }
    /* INTG Customization End. */
